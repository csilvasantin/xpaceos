...
export function mountInventoryBest(container,onReady=()=>{}){
 ...
 async function start(){try{
  const {createLifeRenderer}=await import('./life-renderer.mjs?v=inventari-1');if(disposed)return;
  function tick(now){if(disposed)return;try{if(!document.hidden){const next=now-last>100||!viewer?snapshot(window.__xtancoVisualState?.()):null;
   if(next){label.textContent='BEST · INVENTARIO PBR · '+next.layout.length+' objetos visibles'+(next.moving?' · mudanza':'');if(!viewer){viewer=createLifeRenderer({canvas,snapshot:next,getPlayer:()=>window.__xtoreWindowPlayer});const r=container.getBoundingClientRect();viewer.resize(r.width,r.height);onReady();}else viewer.update(next);last=now;}
   viewer?.render(now);
