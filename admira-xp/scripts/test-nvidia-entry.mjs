...
assert.match(entry, /\/admira-xp\/\?play=shoptalk&amp;lang=en&amp;langlock=1&amp;nvidia=1&amp;build=20260902-1200/);
...
assert.match(app, /document\.documentElement\.classList\.remove\('adm-direct'\)/);
...
assert.match(app, /lang==='en'\?'LOTTERY':'LOTERIAS'/);
assert.match(sitemap, /https:\/\/www\.xpaceos\.com\/Nvidia\//);

console.log('NVIDIA English entry contracts: OK');
