...
function openDetail(a){selected=a;angle=0;$('#detail-name').textContent=a.number+'. '+a.name;$('#detail-category').textContent=a.source?'PIXERIA / SPRITE + RELIEVES 3D':'XPACEOS / MODELO DE MOBILIARIO';$('#detail-previews').replaceChildren(versions(a));showInstances();$('#detail').showModal();}
...
try{
 ({data,stock,registry}=await loadCatalog());merge(stock.items);$('#sync').textContent=assets.length+' piezas con identificadores permanentes · catálogo Pixeria del '+new Date(stock.fetchedAt).toLocaleDateString('es-ES')+'.';
