...
import {STOCK_URL,fromPixeria,instancesFor} from './model.mjs';
const $=s=>document.querySelector(s),store=window.XpaceInventory,tiers=['good','better','best'];
let data,assets=[],category='Todas',selected=null,angle=0,space='xtanco',renderRevision=0;
const cache=new Map();
function el(tag,text,cls){const n=document.createElement(tag);if(text!==undefined)n.textContent=text;if(cls)n.className=cls;return n;}
function layout(){return store.layout(space)||data.layouts[space]||[];}
...
function render(){
 ...
 const query=$('#search').value.trim().toLocaleLowerCase('es');const shown=assets.filter(a=>(category==='Todas'||a.category===category)&&a.name.toLocaleLowerCase('es').includes(query));
 const fragment=document.createDocumentFragment();shown.forEach((a,i)=>{
  const card=el('article',undefined,'card'),head=el('div',undefined,'card-head'),title=el('div');title.append(el('span',a.category+' / '+(a.source||'XpaceOS'),'badge'),el('h2',a.name));head.append(title,el('span',String(i+1).padStart(2,'0'),'number'));
...
function showInstances(){
 ...
 $('#footprint').textContent='Huella '+selected.fp.join(' × ')+' tiles'+(selected.img?' · original + relieves derivados':' · vistas interpretadas');
...
function openDetail(a){selected=a;angle=0;$('#detail-name').textContent=a.name;$('#detail-category').textContent=a.source?'PIXERIA / SPRITE + RELIEVES 3D':'XPACEOS / MODELO DE MOBILIARIO';$('#detail-previews').replaceChildren(versions(a));showInstances();$('#detail').showModal();}
...
function merge(items){const unique=new Map(data.native.map(a=>[a.id,a]));for(const item of items){const a=fromPixeria(item);if(a)unique.set(a.id,a);}assets=[...unique.values()];render();}
$('#refresh').onclick=async()=>{const button=$('#refresh');button.disabled=true;$('#sync').textContent='Consultando Pixeria…';try{const r=await fetch(STOCK_URL,{signal:AbortSignal.timeout(20000),cache:'no-store'});if(!r.ok)throw Error();const d=await r.json();if(!Array.isArray(d.items))throw Error();merge(d.items);$('#sync').textContent=d.items.length+' piezas recibidas de Pixeria'+(d.total>d.items.length?' · el servicio ha limitado la respuesta':'')+'.';}catch{$('#sync').textContent='Pixeria no responde. Conservamos el catálogo cargado; puedes volver a intentarlo.';}finally{button.disabled=false;}};
try{
 const [nativeResponse,pixeriaResponse]=await Promise.all([fetch('./catalog.json'),fetch('./pixeria-cache.json')]);if(!nativeResponse.ok||!pixeriaResponse.ok)throw Error('No se pudo cargar el catálogo');data=await nativeResponse.json();const stock=await pixeriaResponse.json();merge(stock.items);$('#sync').textContent='Copia del catálogo Pixeria · '+new Date(stock.fetchedAt).toLocaleDateString('es-ES')+' · '+stock.items.length+' piezas.';
